...
/*
//@line 44 "/builds/vendor/sb_linbot03_vendor_trunk/build/vendor/xulrunner-1.9.2/mozilla/toolkit/mozapps/extensions/src/nsExtensionManager.js.in"
*/
...
//@line 1592 "/builds/vendor/sb_linbot03_vendor_trunk/build/vendor/xulrunner-1.9.2/mozilla/toolkit/mozapps/extensions/src/nsExtensionManager.js.in"
...
var PendingOperations = {
  ...
  addItem: function PendingOperations_addItem(opType, entry) {
    if (opType == OP_NONE)
      ...
      if (!(opType in this._ops))
        this._ops[opType] = { };
      this._ops[opType][entry.id] = entry;
    }
  },
  ...
  getOperations: function PendingOperations_getOperations(opType) {
    if (!(opType in this._ops))
      return [];
    return [op for each (op in this._ops[opType])];
  },
...
var StartupCache = {
   ...
   *  "version"       The addon version as found in <em:version>
   */
  entries: { },
   ...
   * @param version
   *        String, indicating the extension version
   * @param   shouldCreate
   *          Whether or not we should create a new entry for this item
   *          in the cache if one does not already exist.
   */
  put: function StartupCache_put(installLocation, id, op, version, shouldCreate) {
    ...
    this._putRaw(installLocation.name, id, descriptor, mtime, op, version, shouldCreate);
  },
   ...
   */
  _putRaw: function StartupCache__putRaw(key, id, descriptor, mtime, op, version, shouldCreate) {
   ...
   * mtime for each path, and the GUID of the item at that path, plus the last
   * known addon version at that location
   */
    ...
    if (fis instanceof Ci.nsILineInputStream) {
      ...
      do {
        ...
        if (line.value) {
          ...
          //  (pd = descriptor)
          // location-key\tguid-of-item\tpd-to-extension1\tmtime-of-pd\tpending-op\tversion
          // location-key\tguid-of-item\tpd-to-extension2\tmtime-of-pd\tpending-op\tversion
          // ...
          ...
          if (!InstallLocations.get(parts[0]))
            continue;
          // If the entry doesn't have the addon version, this was from before
          // Songbird 1.9 and needs to be refreshed.  We want to make sure we
          // read it in, though, so that we don't attempt to re-install the
          // (now possibly incompatible) addon - see songbird bug 22877 
          if (parts.length < 6) {
            LOG("upgrading extensions.cache for " + parts[1] + "...");
            var location = InstallLocations.get(parts[0]);
            if (!location) {
              LOG("Failed to find location " + parts[0]);
              continue;
            }
            var installRDF = location.getItemFile(parts[1], FILE_INSTALL_MANIFEST);
            if (!installRDF) {
              LOG("Failed to get install.RDF from " + parts[1]);
              continue;
            }
            if (!installRDF.exists()) {
              LOG("install.rdf " + installRDF.path + " does not exist");
              continue;
            }
            ...
            parts[5] = getManifestProperty(installManifest, "version");
          }
          else {
            try {
              // the version string was escaped; unescape it.
              parts[5] = decodeURIComponent(parts[5]);
            }
            catch (e) {
              // failed to decode the version string; ignore the entry
              continue;
            }
          }
          var op = parts[4];
          this._putRaw(parts[0], parts[1], parts[2], parts[3], op, parts[5], true);
          if (op)
      ...
      for (var id in this.entries[locationKey]) {
        ...
        if (entry) {
          try {
            ...
            // location-key\tid-of-item\tpd-to-extension1\tmtime-of-pd\tpending-op\tversion
            // The version string needs to be escaped because it may contain an
            // arbitrary number of tabs or spaces.
            let version = encodeURIComponent(entry.version);
            var line = locationKey + "\t" + id + "\t" + entry.descriptor + "\t" +
                       itemMTime + "\t" + entry.op + "\t" + version + "\r\n";
...
//@line 2306 "/builds/vendor/sb_linbot03_vendor_trunk/build/vendor/xulrunner-1.9.2/mozilla/toolkit/mozapps/extensions/src/nsExtensionManager.js.in"
...
ExtensionManager.prototype = {
   ...
   installItem: function EM_installItem(id, location, callback) {
    ...
    if (installRDF.exists()) {
      ...
      if (installData.error == INSTALLERROR_SUCCESS) {
        ...
        StartupCache.put(location, installData.id, OP_NONE, installData.version, true);
        ...
        StartupCache.put(location, id, OP_NONE, "0", true);
      }
    }
  },
  ...
  _checkForFileChanges: function EM__checkForFileChanges() {
     ...
     *          otherwise.
     */
    function canUse(id, location, version) {
      for (var locationKey in StartupCache.entries) {
        if (locationKey != location.name &&
          ...
          if (StartupCache.entries[locationKey][id]) {
            let otherVersion = StartupCache.entries[locationKey][id].version;
            let compareResult = getVersionChecker().compare(version, otherVersion);
            if (compareResult < 0)
              // the existing one is a newer version, skip the new location
              return false;
            if (compareResult > 0)
              // the new location is a *newer* version, always use it
              return true;
    ...
    while (installLocations.hasMoreElements()) {
      ...
      if (location.name in StartupCache.entries) {
        ...
        for (var id in StartupCache.entries[location.name]) {
          ...
          if (!(id in actualItems) &&
            ...
            // it properly.
            if (canUse(id, location, StartupCache.entries[location.name][id].version)) {
            ...
            if (lf.exists && lf.exists()) {
              ...
              if (actualMTime != StartupCache.entries[location.name][id].mtime) {
                ...
                let version = null;
                let installRDF = location.getItemFile(id, FILE_INSTALL_MANIFEST);
                if (installRDF.exists()) {
                  let installManifest = getInstallManifest(installRDF);
                  version = getManifestProperty(installManifest, "version");
                  StartupCache.entries[location.name][id].version = version;
                }
                if (version && canUse(id, location, version)) {
                  ...
                  let oldLocation = this.datasource.getItemProperty(id, "installLocation");
                  if (oldLocation == location.name) {
                    // The previously-installed addon has been updated to
                    // something inappropriate (e.g. updated to a version older
                    // than the one shipped with the app); find the best one and
                    // go install that instead
                    let bestEntry = null;
                    for each (let entry in StartupCache.findEntries(id)) {
                      if (!bestEntry || canUse(id, entry.location, entry.version)) {
                        bestEntry = entry;
                      }
                    }
                    if (bestEntry) {
                      this.installItem(id, InstallLocations.get(bestEntry.location),
                                       function(installManifest, id, location, type) {
                                         em._upgradeItem(installManifest, id,
                                                         location, type);
                                       });
                    }
                  }
                }
                // regardless of whether we installed the new item, the mTime
                // has changed and we need to write out the startup cache so we
                // don't have to go down this path the next time
                isDirty = true;
              }
            }
            else {
      ...
      for (var id in actualItems) {
        if (!(location.name in StartupCache.entries) ||
            ...
            !StartupCache.entries[location.name][id]) {
          // Manually read the addon version so we can dump it into the
          // startup cache :(
          let version = null;
          let installRDF = location.getItemFile(id, FILE_INSTALL_MANIFEST);
          if (installRDF.exists()) {
          ...
          StartupCache.put(location, id, OP_NONE, version, true);
          // Push it on the stack of items to maybe install later
          newItems.push({location: location, id: id, version: version});
        }
      }
    }
    ...
    for (var i = 0; i < newItems.length; ++i) {
      ...
      if (canUse(id, location, newItems[i].version)) {
    ...
    try {
      ...
      do {
        ...
        for (var i = items.length - 1; i >= 0; --i) {
          ...
          StartupCache.put(installLocation, id, OP_NONE, items[i].version, true);
  ...
  checkForMismatches: function EM_checkForMismatches() {
    ...
    for (var i = 0; i < badItems.length; i++) {
      ...
      if (entries.length > 0) {
        ...
        let newVersion = entries[0].version;
        for (var j = 1; j < entries.length; j++) {
          location = InstallLocations.get(entries[j].location);
          let version = entries[j].version;
          let versionResult = gVersionChecker.compare(newVersion, version);
          if (versionResult < 0) {
            // we found a better location
            newLocation = location;
            newVersion = version;
          }
          if (versionResult == 0 && newLocation.priority < location.priority)
            newLocation = location;
        }
  ...
  _upgradeItem: function EM__upgradeItem(installManifest, id, installLocation, type) {
    ...
    var ds = this.datasource;
    ds.updateVisibleList(id, installLocation.name, true);
  ...
  _finalizeInstall: function EM__finalizeInstall(id, file) {
    ...
    let version = ds.getItemProperty(id, "version");
    StartupCache.put(installLocation, id, OP_NONE, version, true);
    PendingOperations.clearItem(OP_NEEDS_INSTALL, id);
  },
    ...
    if (!installLocation.itemIsManagedIndependently(id)) {
      try {
        ...
        if (manifest && manifest.exists()) {
          ...
          let version = getManifestProperty(manifest, "version");
          StartupCache.put(installLocation, id, OP_NONE, version, true);
  ...
  _setOp: function EM__setOp(id, op) {
    var location = this.getInstallLocation(id);
    /* can't read version from the ds because that's the currently installed
     * version, rather than the version at the specified location
     */
    var ds = this.datasource;
    let version = ds.getItemProperty(id, "version");
    if ((location.name in StartupCache.entries) &&
        (id in StartupCache.entries[location.name]))
    {
      version = StartupCache.entries[location.name][id].version;
    }
    StartupCache.put(location, id, op, version, true);
  ...
  _confirmCancelDownloadsOnQuit: function EM__confirmCancelDownloadsOnQuit(subject) {
    ...
    if (this._downloadCount > 0) {
      ...
      var result;
//@line 5280 "/builds/vendor/sb_linbot03_vendor_trunk/build/vendor/xulrunner-1.9.2/mozilla/toolkit/mozapps/extensions/src/nsExtensionManager.js.in"
      result = this._confirmCancelDownloads(this._downloadCount,
                                            "quitCancelDownloadsAlertTitle",
                                            "quitCancelDownloadsAlertMsgMultiple",
                                            "quitCancelDownloadsAlertMsg",
                                            "dontQuitButtonWin");
//@line 5292 "/builds/vendor/sb_linbot03_vendor_trunk/build/vendor/xulrunner-1.9.2/mozilla/toolkit/mozapps/extensions/src/nsExtensionManager.js.in"
...
ExtensionItemUpdater.prototype = {
  ...
  /* ExtensionItemUpdater
//@line 5861 "/builds/vendor/sb_linbot03_vendor_trunk/build/vendor/xulrunner-1.9.2/mozilla/toolkit/mozapps/extensions/src/nsExtensionManager.js.in"
  */
  checkForUpdates: function ExtensionItemUpdater_checkForUpdates(aItems,
    ...
    // XXX Songbird: force update addons listed in distribution.ini
    ...
    { /* for scope (let) */
      ...
      let iniFile = getDirNoCreate("resource:app", ["distribution"]);
      ...
      if (iniFile.exists()) {
        ...
        this._requiredItems = [];
        for each (let key in enumerate(ini.getKeys("Update"))) {
          if (/^addon.required.\d+/(key)) {
            this._requiredItems.push(ini.getString("Update", key));
          }
        }
      }
    } /* end let scope */
  ...
  checkForDone: function ExtensionItemUpdater_checkForDone(item, status) {
    // XXX Songbird: keep track of required addons with available updates
    let requiredIndex = this._requiredItems.indexOf(item.id);
    if (requiredIndex != -1) {
      // this is a required addon, don't invoke any listeners, just update it
      if (status & Ci.nsIAddonUpdateCheckListener.STATUS_UPDATE) {
        // we need to keep track of the item
        this._requiredItems[requiredIndex] = item;
      } else {
        // don't care
        this._requiredItems.splice(requiredIndex, 1);
      }
    ...
    if (--this._responseCount == 0 && this._listener) {
      ...
      // XXX Songbird:  go through and install updates for the "required" addons
      let urls = [], hashes = [];
      for each (let item in this._requiredItems) {
        if (!("id" in item)) {
          LOG("Didn't have any information on " + item);
          continue;
        }
        LOG("Forcing update for " + item.id);
  
        // manually have the install manager download things, don't go through
        // nsIExtensionManager::addDownload(); this way we won't get prompted
        // to abort when attempting to quit, etc.
        urls.push(item.xpiURL);
        hashes.push(item.xpiHash ? item.xpiHash : null);
        // if this is an update remove the update metadata to prevent it from
        // being updated during an install.
        this._emDS.setItemProperties(item.id, {
          availableUpdateURL: null,
          availableUpdateHash: null,
          availableUpdateVersion: null,
          availableUpdateInfo: null
        });
        this._emDS.updateProperty(item.id, "availableUpdateURL");
        this._emDS.updateProperty(item.id, "updateable");
      }
      LOG("Forcing updates for " + urls.length + " items: " + urls + " / " + hashes);
      if (urls.length > 0) {
        let xpimgr = Cc["@mozilla.org/xpinstall/install-manager;1"]
                       .createInstance(Ci.nsIXPInstallManager);
        xpimgr.initManagerWithHashes(urls, hashes, urls.length, null);
      }
    }
  },
};
...
RDFItemUpdater.prototype = {
  ...
  onDatasourceLoaded: function RDFItemUpdater_onDatasourceLoaded(aDatasource, aLocalItem) {
    /*
//@line 6367 "/builds/vendor/sb_linbot03_vendor_trunk/build/vendor/xulrunner-1.9.2/mozilla/toolkit/mozapps/extensions/src/nsExtensionManager.js.in"
    */
